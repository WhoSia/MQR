...
fn parse(path:&Path)->Result<Packet,String>{
    ...
    for (i,raw) in text.lines().enumerate(){
        ...
        if cmd.eq_ignore_ascii_case("REALPACKET"){
            if t.len()!=2 || !["0.2","0.3"].contains(&t[1].as_str()){
                return Err(format!("{}:{}: expected REALPACKET 0.2 or 0.3",path.display(),i+1))
            }
            p.version=t[1].clone(); started=true; continue
        }
        if !started{return Err(format!("{}:{}: packet must begin with REALPACKET",path.display(),i+1))}
        ...
        match cmd {
            ...
            "axis" if t.len()>=3 => {
                ...
                let allowed=if p.version=="0.2"{V2_AXES.as_slice()}else{V3_AXES.as_slice()};
                if !allowed.contains(&a.as_str()){
                    return Err(format!("{}:{}: invalid axis {} for v{}",path.display(),i+1,a,p.version))
                }
            ...
            "transport" if p.version=="0.3" && t.len()>=8 => {
                ...
                transport_authority(&eval,&surv).map_err(|e|format!("{}:{}: {}",path.display(),i+1,e))?;
    ...
    if p.id.is_empty()||p.epoch.is_empty()||p.claim_type.is_empty()||p.claim_text.is_empty()||p.scope.is_empty(){
...
fn transport_claim_state(p:&Packet)->String{
    if p.transports.is_empty(){return "NO_EVALUABLE_TARGET".into()}
    let mut any_required=false; let mut any_fail=false; let mut any_hold=false; let mut any_split=false;
    ...
    if !any_required{"NO_EVALUABLE_TARGET".into()}
    else if any_split{"SPLIT_REQUIRED".into()}
    else if any_fail{"SOME_REQUIRED_CELLS_FAIL".into()}
    else if any_hold{"PARTIAL_WITH_HOLDS".into()}
    else{"ALL_REQUIRED_CELLS_PASS".into()}
}
...
fn canonical(p:&Packet,scalar:bool)->String{
    ...
    if p.version=="0.2"{
        ...
        if scalar{
            ...
            o.push(format!("display.scalar.geometric_mean={:.6}",prod.powf(1.0/5.0)));
        ...
        o.push(format!("transport.claim_state={}",transport_claim_state(p)));
        if scalar{
