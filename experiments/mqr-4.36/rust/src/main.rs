...
fn parse(path: &Path) -> Result<Vec<Row>, String> {
    ...
    for (i, line) in s.lines().enumerate() {
        ...
        if p.len() != 8 { return Err(format!("{}:{}: expected 8 columns, got {}", path.display(), i+1, p.len())); }
        out.push(Row {
            case_id: p[0].into(), fiber: p[1].into(), version: p[2].into(),
            probe_class: p[3].into(), probe_id: p[4].into(),
            exact: p[5].into(), coarse: p[6].into(), status: p[7].into(),
        });
    }
    Ok(out)
}
...
fn case_reach(c: &CaseRows, case: &str) -> Result<(String,bool,String,String), String> {
    ...
    for r in &c.rows { versions.insert(r.version.clone()); }
    for v in [a,b,cc] {
        if !versions.contains(v) { return Ok(("E3_UNRESOLVED".into(), false, "HOLD".into(), "HOLD".into())); }
    }

    let mut by: BTreeMap<(String,String,String), &Row> = BTreeMap::new();
    for r in &c.rows {
        by.insert((r.version.clone(), r.probe_class.clone(), r.probe_id.clone()), r);
    }

    let mut core_ids = BTreeSet::new();
    let mut refinement_ids = BTreeSet::new();
    for r in &c.rows {
        if r.probe_class=="CORE" { core_ids.insert(r.probe_id.clone()); }
        if r.probe_class=="REFINEMENT" { refinement_ids.insert(r.probe_id.clone()); }
    }
    ...
    let lookup = |v:&str, k:&str, id:&str| -> Result<&Row,String> {
        by.get(&(v.to_string(),k.to_string(),id.to_string())).copied()
          .ok_or_else(||format!("missing {case}/{v}/{k}/{id}"))
    };

    let mut bridge_ok = true;
    for id in &core_ids {
        let ra=lookup(a,"CORE",id)?; let rb=lookup(b,"CORE",id)?; let rc=lookup(cc,"CORE",id)?;
        if ra.coarse != rb.coarse || rb.coarse != rc.coarse { bridge_ok=false; }
    }
    if !bridge_ok {
        return Ok(("E2_NONE".into(), false, "ACTION_REJECT_REUSE".into(), "REJECT_CORE_REUSE".into()));
    }

    let mut ac_core_coarse = true;
    let mut ac_all_exact = true;
    for id in &core_ids {
        let ra=lookup(a,"CORE",id)?; let rc=lookup(cc,"CORE",id)?;
        if ra.coarse != rc.coarse { ac_core_coarse=false; }
        if ra.exact != rc.exact { ac_all_exact=false; }
    }
    for id in &refinement_ids {
        let ra=lookup(a,"REFINEMENT",id)?; let rc=lookup(cc,"REFINEMENT",id)?;
        if ra.exact != rc.exact { ac_all_exact=false; }
    }

    let reach = if !ac_core_coarse {"E2_NONE"} else if ac_all_exact {"E0_FULL"} else {"E1_QUOTIENT_ONLY"};
    let d36 = match reach {
...
fn main() -> Result<(),String> {
    ...
    if args.is_empty() { return Err("usage: mqr436-adjudicate TSV...".into()); }
    ...
    for a in args {
        for r in parse(Path::new(&a))? {
            ...
            if entry.fiber.is_empty(){entry.fiber=r.fiber.clone();}
            if entry.fiber!=r.fiber{return Err("fiber mismatch".into())}
            entry.rows.push(r);
        }
    }

    let expected=["JSCHEMA-001","OASV-001","UCD2-001","WCWIDTH-001"];
    let mut reach_by_fiber: BTreeMap<String, Vec<(String,String,bool,String,String)>>=BTreeMap::new();
    let mut executed_atoms=0usize;
    let mut unresolved_atoms=0usize;
    for case in expected {
        let c=cases.get(case).ok_or_else(||format!("missing case {case}"))?;
        ...
        println!("CASE={case}\tFIBER={}\tADMITTED={}\tREACH={reach}\tD36={d36}\tD36_CORE={dcore}",c.fiber,admitted);
        reach_by_fiber.entry(c.fiber.clone()).or_default().push((case.into(),reach,admitted,d36,dcore));
    }
    ...
    for (fiber, vals) in &reach_by_fiber {
        ...
        if es.len()>1 {e_collisions+=1; println!("NU3_COLLISION_FIBER={fiber}\tREACHES={}",es.into_iter().collect::<Vec<_>>().join(","));}
        if ds.len()>1 {d36_collisions+=1;}
        if cs.len()>1 {dcore_collisions+=1;}
    }
